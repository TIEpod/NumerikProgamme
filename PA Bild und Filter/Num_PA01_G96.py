...
def frameIt(B,s):
    ...
    J = np.block([[np.flip(B)[-s:, -s:], np.flipud(B)[-s:], np.flip(B)[-s:, :s]],
...
def gewichtererMedian(A, W):
    '''Berechnet den gewichteten Median für die Matrix A mit gewichten W.'''
    gewicht = 0
    fA = A.flatten()                    #Schreibt die Zeilen von A hintereinander entsp. vec(A)^T
    fW = W.flatten()
    fI = np.argsort(fA)                 #fI ist ein Permutiation von 0,...,len(A)-1. In fI sind die
    ak=0                                 #indizes von A gespeichert in der Reihenfolge wenn A sortiert waere.
    for i in range(len(fW)):            #Suchen für die sortierten Indizes den ersten so dass die summe von W
        gewicht = gewicht+fW[fI[i]]     #groesser oder gleich 0.5.
        if gewicht == 0.5:
            k = i
            break
        elif gewicht > 0.5:
...
def mittelwertfilter(A, s=1, type='gleichverteilt', omega=None):
    #Wir erzeugen den gleichverteilten Filter mit groesse passend zu s
    if type=='gleichverteilt':
        W = np.ones((2*s+1, 2*s+1))
        W = W / ((2*s+1)**2)            # W wird normiert, so dass die Summe
    if type=='gauss' and omega:
        M = 3 * math.ceil(omega)
    ...
    for i in range(s, B.shape[0]+s):
        for j in range(s, B.shape[1]+s):
            B[i-s][j-s] = gewichteterMittelwert(J[i-s:i+s+1, j-s:j+s+1],W)
    return B
...
def medianfilter(A, s=1, W=None, type='gleichverteilt',omega=None, saltpepper='out'):
    ...
    elif saltpepper=='active':                   #hat nichts mit aufgabe zu tun
        W[s][s]=0                               # ist saltpepper aktiv, werden nur die Pixsel
...
#io.imshow(imageB2)
#io.show()
C = medianfilter(imageB2, s=2, saltpepper='active')
#C = mittelwertfilter(imageB2,3)
#D = medianfilter(imageB2, s=2)
#C = bilateralerFilter(imageC,1,3,75)
io.imshow(C)
#io.imshow(imageB2)
#io.imshow(D)
#io.imshow(E)
