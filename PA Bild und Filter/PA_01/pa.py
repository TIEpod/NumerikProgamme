...
def mittelwertfilter(B, s , type = 'gleichverteilt', omega = None):
    ...
    if type == 'gleichverteilt':
        ...
        M = 3 * math.ceil(omega)
        W_ = np.zeros(shape=(2 * M + 1, 2 * M + 1))
        for i in range(M + 1):
            for j in range(M + 1):
                W_[i + M][j + M] = math.exp(-(i ** 2 + j ** 2) / (2 * omega ** 2))
                W_[M - i][M - j] = math.exp(-(i ** 2 + j ** 2) / (2 * omega ** 2))
                W_[M - i][M + j] = math.exp(-(i ** 2 + j ** 2) / (2 * omega ** 2))
                W_[M + i][M - j] = math.exp(-(i ** 2 + j ** 2) / (2 * omega ** 2))
        W = W_ / norm(W_)
        if s!=None:
...
def medianfilter(A, s=1, W=None, type='gleichverteilt',omega=None, saltpepper='out'):
    ...
    if W==None and type=='gauss':
        ...
        raise Exception("Die Summe der Einträge von W ist nicht Eins. ")

    J = frameIt(A,s)
    B = np.copy(A)

    # Zusatz: Ist saltpepper aktiv, werden nur die Picksel
    # bearbeitet die schwarz(255) oder weiss(0) sind.
    ...
    elif saltpepper=='activ':
        W[s][s]=0#Der Schwarz oder Weisse Pixel soll nun nicht mehr bewertet werden
        W = np.multiply(W, 1/norm(W))
...
#D = medianfilter(imageB2, s=2, saltpepper='activ')
#D = mittelwertfilter(imageB2,3)
#D = medianfilter(imageB2, s=2)
#D = bilateralerFilter(imageC,2,3,75)
